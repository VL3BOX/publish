<template>
	<div class="m-publish-box">
		...
		<el-form label-position="left" label-width="80px" class="m-publish-item-plan">
			<!-- 清单名称 -->
			<el-form-item label="标题">
				<el-input v-model="plan.title" placeholder="请输入物品清单的标题" maxlength="20" show-word-limit></el-input>
			</el-form-item>
			...
			<el-form-item label="描述">
				<el-input type="textarea" :rows="3" v-model="plan.description" placeholder="简单说明一下你的物品清单" maxlength="2000" show-word-limit></el-input>
			</el-form-item>
			<!-- 清单类型 -->
						...
						<el-row :gutter="15" class="m-positions">
							<el-col :xs="24" :sm="6" v-for="(position, key) in normal_relation" :key="key">
								<div class="m-position">
									<el-input class="u-title" v-model="position.title" placeholder="子清单标题（选填）" maxlength="20" show-word-limit></el-input>
									<div class="c-selected-items">
										<draggable
											...
											:class="{
												empty: !position.data || (position.data && !position.data.length),
											}"
											ghost-class="ghost"
										>
											<li v-for="(item, k) in position.data" :key="k" class="m-selected-item">
												<!-- <jx3-item-simple :item="item" /> -->
												<div class="u-selected">
													<span class="u-name" :class="`quality-${item.Quality}`"> <img class="u-icon" :src="`${icon}/icon/${item.IconID}.png`" alt="" />{{ item.Name }}</span>
													<input class="u-input" placeholder="数量" v-model="item.num" />
												</div>

												<i class="u-el-icon el-icon-circle-close" @click="position.data.splice(k, 1)"></i>
											</li>
										</draggable>
										<span v-if="!position.data || (position.data && !position.data.length)" class="u-tip">拖拽所需道具到此处</span>
									</div>
									<i class="u-selected-icon el-icon-circle-close" @click="normal_relation.splice(key, 1)"></i>
								</div>
							</el-col>
						</el-row>
						<el-row :gutter="15">
								...
								<div class="m-position" v-for="(position, key) in _positions" :key="key">
									...
									<div class="c-selected-items">
										<draggable
											...
											:data-AucSubType="position.AucSubType"
											:class="{
												empty: !equip_relation[key] || (equip_relation[key] && !equip_relation[key].length),
											}"
											ghost-class="ghost"
										>
										...
										<span v-if="!equip_relation[key] || (equip_relation[key] && !equip_relation[key].length)" class="u-tip">拖拽所需装备到此处</span>
									</div>
								</div>
							</el-col>
						</el-row>
			...
			<el-form-item>
				<el-button class="u-publish" icon="el-icon-s-promotion" type="primary" @click="submit" :loading="processing">提交物品清单 </el-button>
			</el-form-item>
		</el-form>
	</div>
</template>
...
export default {
	...
	methods: {
		...
		submit() {
			...
			console.log(this.plan.relation, this.plan.type, "this.plan.relation ");
			return console.log(this.plan, "this.plan");
			this.processing = true;
			save_item_plan(this.plan)
				.then((data) => {
					data = data.data;
					if (data.code === 200) {
