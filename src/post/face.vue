            ...
            <div class="m-publish-info">
                ...
                <el-form-item label="数据">
                    ...
                    <div class="u-face-info" v-if="faceData">
                        {{`${maps.
                        client[post.client]}·${maps.
                        roleType[post.body_type]}·${post.
                        is_new_face === 1 ? '写实' : '写意'}·${post.
                        is_unlimited === 1 ? '可新建' : '不可新建'}`}}
                        ...
                        <template v-if="editDetail">
                            ...
                            <publish-client v-if="faceData" v-model="post.client" :forbidAll="true">
                                <template v-if="post.client === 'std'">
                                    <el-form-item label="   ">
...
export default {
  ...
  data() {
    ...
    handleFaceChange({ object = null, json = "", uuid = "", id = "", name = "" }) {
      ...
      if (true || !this.faceData) {
        this.faceData = {
          ...
          this.post.client = object.nMajorVersion === 2 ? 'std' : 'origin';
    ...
    setMain(item) {
        ...
        this.post.client = JSON.parse(item.data)
          .nMajorVersion === 2 ? 'std' : 'origin';
        this.post.is_new_face = JSON.parse(item.data)
          .bNewFace ? 1 : 0;
        this.post.body_type = JSON.parse(item.data)
            .nRoleType;
        this.post.is_unlimited = ~~this.decalDb.canUseInCreate(      JSON.parse(item.data));
        this.post.game_price = ~~this.decalDb.getTotalPrice(JSON.parse(item.data))
        this.$notify({
        title: "设置成功",
        type: "success",
        duration: 2000,
